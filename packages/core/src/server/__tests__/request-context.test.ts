...
describe('createRequestContext', () => {
  it('returns a valid UUID for requestId', () => {
    const ctx = createRequestContext(mockReq('/test'), undefined, new URL('http://localhost:3000/test'))
  ...
  it('generates different requestId each call', () => {
    const a = createRequestContext(mockReq('/'), undefined, new URL('http://localhost:3000/'))
    const b = createRequestContext(mockReq('/'), undefined, new URL('http://localhost:3000/'))
    expect(a.requestId).not.toBe(b.requestId)
  })

  it('startTime is a 2-element tuple', () => {
    const ctx = createRequestContext(mockReq('/'), undefined, new URL('http://localhost:3000/'))
  ...
  it('url is parsed from req.url', () => {
    const ctx = createRequestContext(mockReq('/hello?foo=bar'), undefined, new URL('http://localhost:3000/hello?foo=bar'))
  ...
  it('params passed in are preserved', () => {
    const ctx = createRequestContext(mockReq('/users/42'), { id: '42' }, new URL('http://localhost:3000/users/42'))
